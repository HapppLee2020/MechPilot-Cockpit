"""Deploy PDM vault icon changes to target D:\\SWAgentAddin via MCP."""
...
DEPLOY_FILES = [
    ...
    (r"EPDM.Interop.epdm.dll", r"EPDM.Interop.epdm.dll"),
...
async def main() -> bool:
    ...
    archive_dir = fr"{Z_ARCHIVE_BASE}\{ts}\Cockpit-pdm-icon"
    report = {"archive_dir": archive_dir, "lifecycle": [], "backups": [], "deploys": [], "protected": [], "errors": []}

    for src_rel, _ in DEPLOY_FILES:
        src_path = os.path.join(Z_DEPLOY, src_rel)
        if not os.path.isfile(src_path):
            report["errors"].append(f"missing Z source: {src_path}")
